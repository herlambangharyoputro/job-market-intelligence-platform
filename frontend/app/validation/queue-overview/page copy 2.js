...
export default function QueueOverviewPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setPendingSkills(queueResponse.data.items)
    } catch (error) {
  ...
  const handleResetStuck = async () => {
    try {
      ...
      toast.success(`✅ Reset ${response.data.count} stuck items`)
      loadData()
    } catch (error) {
      const apiError = handleApiError(error)
      toast.error(apiError.message)
  ...
  const handleReprioritize = async () => {
    try {
      ...
      toast.success(`✅ Reprioritized ${response.data.count} items`)
      loadData()
    } catch (error) {
      const apiError = handleApiError(error)
      toast.error(apiError.message)
  ...
  const handleUpdate = async () => {
    try {
      // Note: Backend endpoint needs to support queue update
      // For now, we'll show toast (implement backend endpoint first)
      toast.info('Queue item update feature - implement backend endpoint')
      setShowEditDialog(false)
      
      // TODO: Uncomment when backend endpoint ready
      // await validationApi.updateQueueItem(selectedItem.id, editForm)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          <Button variant="outline" onClick={handleResetStuck}>
            <RefreshCw className="h-4 w-4 mr-2" />
            Reset Stuck
          </Button>
          <Button variant="outline" onClick={handleReprioritize}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <Card>
          <CardContent className="pt-6">
            <div className="text-2xl font-bold">{stats.total_queue_items || 0}</div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 border border-blue-200 rounded p-3">
                <p className="text-sm text-blue-800">
                  <strong>Note:</strong> This feature requires backend implementation. 
                  Currently showing UI only.
                </p>
              </div>
            </div>
          )}
